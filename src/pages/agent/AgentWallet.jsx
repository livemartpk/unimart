// ============================================
// UniMart - Wallet & Ledger (Agent)
// Same Gross/Net pattern as Seller Wallet.
// ============================================
...
export default function AgentWallet({ user, country, onNavigate }) {
  ...
  const loadWallet = async () => {
    ...
    try {
      ...
      const grossSnap = await getDocs(query(collection(db, "wallets_agent", user.uid, "ledger_gross"), orderBy("createdAt", "desc")));
      setGrossLedger(grossSnap.docs.map((d) => ({ id: d.id, ...d.data() })));

      const netSnap = await getDocs(query(collection(db, "wallets_agent", user.uid, "ledger_net"), orderBy("createdAt", "desc")));
  ...
  const handleWithdrawRequest = async () => {
    ...
    setSubmitting(true);
    try {
      await addDoc(collection(db, "wallets_agent", user.uid, "ledger_net"), {
      ...
      <div className="container" style={{ paddingTop: 16 }}>
        <div style={styles.balanceRow}>
          ...
          <div style={{ ...styles.balanceCard, background: "#0B3D2E" }}>
            <div style={{ ...styles.balanceLabel, color: "#cfe0d4" }}>Available</div>
        ...
        <div style={styles.infoNote}>
          ℹ️ Commission appears in <b>Gross Ledger</b> when a tagged order is dispatched, and moves to <b>Net Ledger</b> once the buyer confirms delivery (or the policy window passes automatically).
        </div>
            ...
            <div key={entry.id} style={styles.ledgerRow}>
              <div>
                <div style={styles.ledgerType}>
                  {entry.type === "withdrawal_request" ? "Withdrawal Request" : entry.source === "referral" ? "Referral Sale" : "Tagged Store Sale"}
                </div>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                {entry.status && (
                  <div style={{ ...styles.ledgerStatus, color: entry.status === "paid" ? "#2E7D32" : "#D4AF37" }}>{entry.status}</div>
                )}
              </div>
            </div>
          ))
      ...
      {showWithdrawModal && (
        <div style={styles.modalOverlay} onClick={() => setShowWithdrawModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <input className="input-field" style={{ marginBottom: 16 }} value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} placeholder="Easypaisa / JazzCash account" />
